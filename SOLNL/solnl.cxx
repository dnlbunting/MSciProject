...
protected:
  ...
  int rhs(BoutReal t) override {
    ...
    n(0,0,0) = 1; n(0,1,0) = 1;
    n(0,N+2,0) = 1; n(0,N+3,0) = 1;

    v(0,0,0) = std::max(-r*t, -1.); v(0,1,0) = std::max(-r*t, -1.); v(0,2,0) = std::max(-r*t, -1.);
    v(0,N+2,0) = std::min(r*t, 1.); v(0,N+3,0) = std::min(r*t, 1.);
    ...
    Field3D S = t < 1./r ? r*t*S_n : S_n;
    // Fluid equations
    ddt(n) =  c_st * (S - VDDY(v, n, CELL_CENTRE) - n*DDY(v, CELL_CENTRE));
    ddt(v) = (-DDY(p, CELL_YLOW))/(m_i*n*n_t*c_st) - c_st*(2 * VDDY(v, v, CELL_YLOW)  +  v*(VDDY(v, n, CELL_YLOW)/n));
    n.applyTDerivBoundary();
    ddt(T) = 0;//(1 / (3 * n_t*n * SI::qe)) * ( S_u - DDY(q, CELL_CENTRE, DIFF_C2) ); //+ VDDY(v,p, CELL_CENTRE)) + (T/n) * ddt(n);

    A=VDDY(v, n, CELL_CENTRE, DIFF_U4);
    B=VDDY(v, n, CELL_CENTRE, DIFF_U2);
    C=0;//(S_n - VDDY(v, n, CELL_CENTRE, DIFF_U2) - n*DDY(v, CELL_CENTRE));
